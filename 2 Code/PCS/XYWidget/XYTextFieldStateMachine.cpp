 ...
 * All rights reserved.
 *
 * @file    XYTextFieldStateMachine.cpp
 *
 * @example
 *
 * @brief
...
XYTextFieldStateMachine::XYTextFieldStateMachine(XYTextFieldLineEdit *parent)
...
XYTextFieldStateMachine::~XYTextFieldStateMachine()
{
}

void XYTextFieldStateMachine::setLabel(XYTextFieldLabel *label)
{
    if (m_label)
    {
        delete m_label;
...
void XYTextFieldStateMachine::setupProperties()
{
    if (m_label)
    {
...
XYTextFieldLabel::XYTextFieldLabel(XYTextFieldLineEdit *parent)
    : QWidget(parent)
    , textField(parent)
    , labelScale(1)
    , labelPosX(0)
    ...
    , labelColor(parent->labelColor())
{
    ...
    font.setPointSizeF(parent->labelFontSize());
    font.setWeight(QFont::Medium);
    font.setLetterSpacing(QFont::PercentageSpacing, 102);
    setFont(font);
}

XYTextFieldLabel::~XYTextFieldLabel()
{
}

void XYTextFieldLabel::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
    ...
    painter.scale(labelScale, labelScale);
    painter.setPen(labelColor);
    painter.setOpacity(1);

    QPointF pos(2 + labelPosX, height() - 36 + labelPosY);
    painter.drawText(pos.x(), pos.y(), textField->label());
}
